...
**** Leader keys
     ...
     | SPC a e   | lsp                         | Connect to LSP server  |
     ...
     | SPC c A   | lsp-format-region                    | Format region               |
     | SPC c c   | compile                               | Compile project             |
     | SPC c C   | recompile                             | Re-run last compile         |
     | SPC c D   | lsp-find-declaration                 | Go to declaration           |
     ...
     | SPC c F   | lsp-organize-imports                 | Organize imports            |
     | SPC c i   | consult-imenu                         | Buffer outline              |
     | SPC c I   | consult-imenu-multi                   | Multi-buffer outline        |
     | SPC c l   | lsp                                   | Connect LSP                 |
     | SPC c R   | lsp-restart-workspace                 | Reconnect server            |
     | SPC c S   | lsp-workspace-shutdown               | Shutdown server             |
...
**** LSP helpers
    ...
    LSP interactions share a consistent local map so language servers expose the same motions everywhere.
     ...
     #+caption: `SPC m` — LSP helpers
     | Key         | Command                                | Description            |
     |-------------+----------------------------------------+------------------------|
     | SPC m a     | lsp-execute-code-action                | Code actions           |
     | SPC m d     | flymake-show-buffer-diagnostics        | Buffer diagnostics     |
     | SPC m D     | flymake-show-project-diagnostics       | Project diagnostics    |
     | SPC m f     | lsp-format-region                      | Format region          |
     | SPC m F     | lsp-format-buffer                      | Format buffer          |
     | SPC m h     | lsp-ui-doc-show                        | Hover help             |
     | SPC m o     | lsp-organize-imports                   | Organize imports       |
     | SPC m r     | lsp-rename                             | Rename symbol          |
     | SPC m R     | lsp-restart-workspace                  | Restart workspace      |
     | SPC m s     | lsp-workspace-shutdown                 | Shutdown workspace     |
#+begin_src emacs-lisp
...
** Programming
   ...
   Language tooling centres around `lsp-mode` for LSP support, with tree-sitter text objects, lsp-ui diagnostics, and language-specific helpers.
*** consult-lsp
    [[https://github.com/alphapapa/consult-lsp][consult-lsp]] bridges Consult's UI with `lsp-mode` symbol search so workspace definitions stay easy to browse.
#+begin_src emacs-lisp
...
*** lsp-mode
    [[https://github.com/emacs-lsp/lsp-mode][lsp-mode]] wires Emacs to language servers and exposes hooks for completion, diagnostics, and IDE-like helpers.
#+begin_src emacs-lisp
...
*** lsp-ui
    [[https://github.com/emacs-lsp/lsp-ui][lsp-ui]] enriches the hover, peek, and sideline UX that the LSP backends expose.
#+begin_src emacs-lisp
(use-package lsp-ui
  :straight t
  :after lsp-mode
...
*** JSON
    JSON files lean on tree-sitter remaps, jq utilities, and LSP-backed validation so payloads stay tidy and inspectable.
#+begin_src emacs-lisp
...
*** Python
    Python support stitches together tree-sitter remaps, auto-venv activation, `lsp-mode` (Pyright) workspaces, Black/Isort formatting, [[https://github.com/emacs-lsp/dap-mode][DAP mode]], and pytest helpers.
    #+begin_src emacs-lisp
    (require 'project)
