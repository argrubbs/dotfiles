...
which-key is built into Emacs 30+. It displays available keybindings in a
popup after a short delay, making standard Emacs bindings (=C-x=, =C-c=,
=C-h=, etc.) fully discoverable.
...
(use-package which-key
  ...
  (which-key-add-key-based-replacements
    ...
    "C-c n i" "roam-insert-link"
    "C-c n c" "roam-capture"
    "C-c n d" "roam-daily"
    "C-c p"   "completion-at-point"
    "C-c p p" "complete-at-point"
    "C-c p f" "complete-file"
    "C-c p d" "complete-dabbrev"
    "C-c p k" "complete-keyword")

  ;; Label C-h (help) group — overridden by helpful below
  (which-key-add-key-based-replacements
    "C-h"     "help"
  ...
  ;; Label M-g (goto) group
  (which-key-add-key-based-replacements
    "M-g"     "goto"
    "M-g g"   "goto-line"
    "M-g M-g" "goto-line"
    "M-g e"   "consult-errors"
...
#+begin_src emacs-lisp
        (when (member "Cascadia Code NF" (font-family-list))
          (set-face-attribute 'default nil :family "Cascadia Code NF" :height 160 :weight 'semi-light)
        ...
        (when (member "Cascadia Code NF" (font-family-list))
          (set-face-attribute 'variable-pitch nil :family "Cascadia Code NF" :height 170))
      ...
      (when (member "Cascadia Code NF" (font-family-list))
        (dolist (face '((org-level-1 . 1.35)
                        ...
                        (org-level-7 . 1.1)
                        (org-level-8 . 1.1)))
          (set-face-attribute (car face) nil :font "Cascadia Code NF" :weight 'bold :height (cdr face)))

        ;; Make the document title a bit bigger
        (set-face-attribute 'org-document-title nil :font "Cascadia Code NF" :weight
        'bold :height 1.8))
...
[[https://github.com/copilot-emacs/copilot.el][copilot.el]] provides inline ghost-text completions and an interactive chat
interface backed by the official =@github/copilot-language-server=. After
installing, run =M-x copilot-install-server= once to fetch the language server
binary, then =M-x copilot-login= to authenticate.

**** Inline completions

Completions activate in all =prog-mode= buffers. TAB accepts a full suggestion
(taking priority over Corfu when an overlay is visible); =C-n= / =C-p= cycle
through alternatives; =M-f= accepts by word.
...
=copilot-chat= opens a streaming chat buffer in a right side window. =C-c c c=
starts a new chat; =C-c c r= sends the selected region as context.

#+begin_src emacs-lisp
(with-eval-after-load 'copilot
  (global-set-key (kbd "C-c c c") #'copilot-chat)
  (global-set-key (kbd "C-c c r") #'copilot-chat-send-region)
...
[[https://github.com/Wilfred/helpful][helpful]] replaces the built-in help buffers with richer ones that show source
code, callers, keybindings, and links to the Info manual all in one place.

#+begin_src emacs-lisp
(use-package helpful
  :demand t
  :bind (;; Replace standard C-h bindings
         ...
         ("C-h C-d" . helpful-at-point))
  :config
  ;; Make describe-* in other packages also use helpful
