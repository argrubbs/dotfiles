...
*** Doom Snippets (doomemacs/snippets)

High-quality, comprehensive snippets covering many languages and frameworks.
Not on MELPA; install directly from Git (Emacs 29+ provides package-vc).
...
(with-eval-after-load 'yasnippet
  ...
  (when (and (not (package-installed-p 'doom-snippets))
    ...
    ;; Add package-generated files to gitignore to prevent checkout conflicts
    (let ((snippets-dir (locate-library "doom-snippets")))
      (when snippets-dir
        (let ((gitignore-path (expand-file-name ".gitignore"
                                                (file-name-directory snippets-dir))))
          (with-temp-buffer
            (when (file-exists-p gitignore-path)
              (insert-file-contents gitignore-path))
            (goto-char (point-max))
            (unless (search-backward "snippets-autoloads.el" nil t)
              (when (> (point-max) 1)
                (insert "\n"))
              (insert "# Package-generated files\n")
              (insert "snippets-autoloads.el\n")
              (insert "snippets-pkg.el\n"))
