...
;; Enhanced Ansible support with proper mode detection
(defun my/is-ansible-file ()
  "Check if current file is an Ansible file."
  (when buffer-file-name
    (or
     ;; Ansible directory patterns
     (string-match-p "/\\(group\\|host\\)_vars/" buffer-file-name)
     (string-match-p "/playbooks/.*\\.ya?ml\\'" buffer-file-name)
     (string-match-p "/roles/.*/\\(tasks\\|handlers\\|vars\\|defaults\\|meta\\)/.*\\.ya?ml\\'" buffer-file-name)
     (string-match-p "ansible.*\\.cfg\\'" buffer-file-name)
     (string-match-p "/inventory" buffer-file-name)
     ;; Common Ansible filenames
     (string-match-p "\\(playbook\\|site\\|main\\|local\\)\\.ya?ml\\'" buffer-file-name)
     ;; Check file contents for Ansible indicators
     (and (derived-mode-p 'yaml-mode)
          (save-excursion
            (goto-char (point-min))
            (or (re-search-forward "^\\s-*-\\s-*hosts:\\s-*" nil t)
                (re-search-forward "^\\s-*-\\s-*name:\\s-*" nil t)
                (re-search-forward "^\\s-*ansible_" nil t)
                (re-search-forward "^\\s-*gather_facts:" nil t)
                (re-search-forward "^\\s-*vars:" nil t)))))))

(use-package ansible
  :ensure t
  :hook ((yaml-mode . (lambda ()
                        (when (my/is-ansible-file)
                          (ansible-mode 1))))
         (ansible-mode . ansible-auto-decrypt-encrypt))
  :custom
  (ansible-vault-password-file "~/.ansible-vault")  ; Adjust path as needed
  :config
  ;; Auto-detect Ansible files and set proper modes
  (add-to-list 'auto-mode-alist '("/\\(group\\|host\\)_vars/.*\\.ya?ml\\'" . yaml-mode))
  (add-to-list 'auto-mode-alist '("/playbooks/.*\\.ya?ml\\'" . yaml-mode))
  (add-to-list 'auto-mode-alist '("/roles/.*/\\(tasks\\|handlers\\|vars\\|defaults\\|meta\\)/.*\\.ya?ml\\'" . yaml-mode))
  (add-to-list 'auto-mode-alist '("\\(playbook\\|site\\|main\\|local\\)\\.ya?ml\\'" . yaml-mode))
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '((ansible-mode yaml-mode) . ("ansible-language-server" "--stdio")))
    ;; Auto-start eglot for Ansible files
