...
;; Web development - HTML with embedded CSS/JS
(use-package poly-web
  :ensure t
  :after polymode
  :config
  ;; HTML with embedded CSS and JavaScript
  (add-to-list 'auto-mode-alist '("\\.html\\'" . poly-html-mode))
  (add-to-list 'auto-mode-alist '("\\.htm\\'" . poly-html-mode))

  ;; Vue.js single file components
  (add-to-list 'auto-mode-alist '("\\.vue\\'" . poly-vue-mode))

  ;; Django templates
  (add-to-list 'auto-mode-alist '("\\.djhtml\\'" . poly-django-html-mode)))

;; Python + various embedded languages
(use-package poly-python
  :ensure t
  :after polymode
  :config
  ;; Jupyter notebooks (if using python-mode for .py files with cells)
  ;; SQL in Python strings
  (define-auto-innermode poly-python-sql-innermode
    :mode 'sql-mode
    :head-matcher "\\(sql\\s-*=\\s-*\\|execute\\s-*(\\s-*\\)[\"']"
    :tail-matcher "[\"']"
    :head-mode 'host
    :tail-mode 'host))

;; Dockerfile with embedded shell scripts
(use-package poly-dockerfile
  :ensure t
  :after polymode
  :config
  (add-to-list 'auto-mode-alist '("Dockerfile\\'" . poly-dockerfile-mode))
  (add-to-list 'auto-mode-alist '("\\.dockerfile\\'" . poly-dockerfile-mode)))
